#!/usr/bin/env python3
"""dais dashboard — the renderers + the TUI action engine, on the board.py data layer.

Powers `dais status` (plain one-shot), `dais project` and `dais top` (curses TUI —
the base `App` here; the cockpit renderer is panel.PanelApp, which main() launches).
Read-only: this module never writes dais.db (task changes go through `dais fire`).
"""
import curses
...
# the sibling modules live beside this file in harness/; `dais top` runs this module
# as a script (so harness/ is sys.path[0]) and the tests insert harness/ on the path,
# so bare imports resolve in both.
...
class App:
    ...
    # ---- the renderer contract — what a subclass must supply for run() to work ----
    def draw(self):
        raise NotImplementedError("App is the engine half — a renderer subclass "
                                  "(panel.PanelApp) supplies draw()")

    def left_rows(self):
        raise NotImplementedError("App is the engine half — a renderer subclass "
                                  "(panel.PanelApp) supplies left_rows()")
